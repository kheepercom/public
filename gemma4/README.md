...
| Field | Description |
| ----- | ----------- |
| `admin_authorized_keys` | SSH public key(s), one per line. Required so you can `ssh admin@<host>` to debug or check logs while the model loads. Inherited from the base image. |
| `domain` | Public domain resolving to this host. Used by Caddy for automatic TLS. After your host auto-registers it will have a DNS record at `<host>.<org>.kheeper.app` you can use. |
| `api_key` | Bearer token clients send as `Authorization: Bearer <api_key>`. Minimum 24 characters; generate with `openssl rand -hex 24`. **Rotation** requires editing `config.json` and restarting `gemma4-vllm.service`. |
| `max_model_len` | Optional, default 32768. vLLM context window in tokens (1024..131072). |
...
jq --arg keys "$(cat ~/.ssh/id_ed25519.pub)" \
   ...
   '.base.admin_authorized_keys=$keys
    | .gemma4.domain=$domain
    | .gemma4.api_key=$api_key' \
...
```
ssh admin@$HOST.$ORG.kheeper.app sudo journalctl -fu kheeper-upgrade -u gemma4-vllm
```
...
Grafana lives at `https://<your-domain>/grafana/` (default credentials `admin` / `admin`; Grafana forces a password change on first login). The bundled "Gemma 4 vLLM" dashboard shows tokens/sec, time-to-first-token, end-to-end latency, active/queued requests, KV cache utilization, and request failure rate.
